...
for i, pattern in enumerate(patterns):
    ...
    nine = [word for word in pattern if len(word) == 6 and len(not_coinciding_wire(word,union(four,seven)))==1][0]
    ...
    six = [word for word in pattern if len(word) == 6 and len(not_coinciding_wire(eight,union(word, one)))==0][0]
    ...
    total += int("".join([matching_patterns[sort_string(out)] for out in output[i]]))
print("Part 2", total)
